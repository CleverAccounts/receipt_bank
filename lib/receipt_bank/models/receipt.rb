module ReceiptBank
  ...
  module Models
    ...
    class Receipt < ReceiptBank::Models::BaseModel
      ...
      def get_image
        options = { sessionid:self.session[:session_id], id: self.id }
      ...
      def save
        return save_new_image if !has_attribute?("id") && self.local_image_file_path && !self.local_image_file_path.empty?
        options = { data: build_changed_data.to_json, id: self.id }.merge(build_session_data)
        response = self.client_connection.query_post_api(self.class.build_url(:update_receipt_data), options)
      ...
      def save_new_image
        response = self.class.upload_receipt(client_connection.current_user,
                                    self.local_image_file_path)

        self.set_attribute("id", response["receiptid"], true)
        self
      end
